...
def userregisterdetails(request):
    ...
    u1.save()
    #print(name,cno,email,add,username,password)
    return render(request,"UserRegistration.html")
...
def a_vegdetails(request):
    a_type=request.GET.get("type")

    return render(request,"admin/Admin_itemAdd.html",{"type":a_type})


def viewnonvegdetails(request):
    a_type = request.GET.get("type")
